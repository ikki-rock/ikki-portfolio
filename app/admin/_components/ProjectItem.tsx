...
import { Edit2 } from "lucide-react"; // 아이콘 소환 🥊
...
export default function ProjectItem({ project, className }: ProjectItemProps) {
  const { title, mode, desc, tags, role, thumbnail, updated_at } = project;
      ...
      <CardHeader className="p-4 space-y-1 flex-1">
        <div className="flex justify-between items-start gap-2">
          ...
          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
            <button className="p-1.5 hover:bg-secondary rounded-md transition-colors text-muted-foreground hover:text-foreground">
              <Edit2 size={14} />
            </button>
          </div>
        </div>
